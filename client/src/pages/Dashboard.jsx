...
export default function Dashboard({ user, message = '' }) {
  ...
  if (!data) {
    return (
      <>
        <PageHeader title="Dashboard Principal" subtitle="Carregando indicadores em segundo plano." />
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {Array.from({ length: 8 }).map((_, index) => (
            <div key={index} className="h-24 animate-pulse rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
